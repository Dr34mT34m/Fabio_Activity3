...
circular_operation Right_Wheel(RIGHT_WHEEL), Left_Wheel(LEFT_WHEEL);
...
void loop() {
  if (Right_Wheel.isTimeToTakeMeasurement() && Left_Wheel.isTimeToTakeMeasurement()){     //drives car in a circular motion clockwise, then anti-clockwise
    Right_Wheel.execute_system_task_circular(100, 125, 3, 300, 125, 2, STOP_TIME_MS, 2);                       
    Serial.print(",");                                                                   
    Left_Wheel.execute_system_task_circular(100, 125, 3, 300, 125, 2, STOP_TIME_MS, 2); 
    Serial.println("");
  }
}
